...
public class BreakoutFinder {
    ...
    @Autowired
    BreakoutFinderIndicator breakoutFinderIndicator;
    ...
    public void run(BybitKlineResponse response, String timeframe) throws TelegramApiException {
        ...
        if (mainTfSignal != 0){
            ...
            if (!openPositions.getSymbols().containsKey(symbol) && openPositions.getSymbols().size() < settings.activeTrades) {
                ...
                if (1 == mainTfSignal && 1 == lowTfSignal && opens.get(0) < closingPrices.get(0)) { //BUY
                    TradeLevelsCalculator.TradeLevels levels = levelsCalculator.calculateTradeLevelsForLevel(lowTFCandles,
                ...
                } else if (-1 == mainTfSignal && -1 == lowTfSignal && opens.get(0) > closingPrices.get(0)) { //SELL
                    TradeLevelsCalculator.TradeLevels levels = levelsCalculator.calculateTradeLevelsForLevel(lowTFCandles,
